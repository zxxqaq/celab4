...
static inline iss_reg_t lwi_exec(Iss *iss, iss_insn_t *insn, iss_reg_t pc)
{

    iss->regfile.memcheck_access_reg(REG_IN(0));
    iss->regfile.memcheck_access_reg(REG_IN(1));

    iss_addr_t effective_address = REG_GET(0) + REG_GET(1);

    iss->lsu.stack_access_check(REG_IN(0), effective_address);

    if (iss->lsu.load_signed_perf<int32_t>(insn, effective_address, 4, REG_OUT(0)))
    {
        return pc; 
    }
